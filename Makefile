...
OPENCODE_VERSION ?= 1.15.6
CRUSH_VERSION    ?= 0.70.0
GH_VERSION       ?= 2.92.0
FZF_VERSION      ?= 0.72.0
RG_VERSION       ?= 15.1.0
...
update-deps:  ## Fetch latest versions of all dependencies and update Makefile
	...
	$(eval LATEST_MAKE_LS := $(shell curl -fsSL 'https://api.github.com/repos/owenrumney/make-ls/releases/latest' | jq -r '.tag_name | ltrimstr("v")'))
	$(eval LATEST_OPENSHELL := $(shell curl -fsSL 'https://api.github.com/repos/NVIDIA/OpenShell/releases/latest' | jq -r '.tag_name | ltrimstr("v")'))
	@echo "Go: $(LATEST_GO)  Python: $(LATEST_PY) (build: $(LATEST_BUILD))  opencode: $(LATEST_OC)  crush: $(LATEST_CRUSH)  gh: $(LATEST_GH)  fzf: $(LATEST_FZF)  rg: $(LATEST_RG)  jira-mcp: $(LATEST_JIRA_MCP)  gopls: $(LATEST_GOPLS)  pyright: $(LATEST_PYRIGHT)  make-ls: $(LATEST_MAKE_LS)  openshell: $(LATEST_OPENSHELL)"
